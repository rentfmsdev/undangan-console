  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!response.ok || !payload.user) {
        throw new Error("Sesi Google belum dapat dimuat.");
      }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      {isAuthModalOpen && (
        <div
          ...
          aria-modal="true"
          onClick={() => {
            if (!isGoogleLoginLoading) setIsAuthModalOpen(false);
          }}
        >
          <div
          ...
          >
            <div className="flex justify-end">
              <button
                ...
                className="rounded-full p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700 disabled:cursor-wait disabled:opacity-50"
            ...
            <div className="mt-6">
              <button
                ...
                disabled={isGoogleLoginLoading}
                className="flex w-full items-center justify-center gap-3 rounded-2xl border border-slate-300 bg-white px-5 py-3.5 text-xs font-extrabold text-slate-800 shadow-sm transition hover:border-slate-400 hover:bg-slate-50 hover:shadow-md active:scale-95 disabled:cursor-wait disabled:opacity-70"
              >
                {isGoogleLoginLoading ? (
                  <LoaderCircle size={19} className="animate-spin text-emerald-600" />
                ) : (
                ...
                <span>{isGoogleLoginLoading ? "Menghubungkan..." : "Lanjutkan dengan Google"}</span>
              </button>
            </div>

            {googleLoginError && (
              <div className="mt-3 flex items-start gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2.5 text-left text-xs font-semibold text-rose-700">
      ...
      <a
        href={makeAdminWhatsAppUrl(
          "Halo Admin Undangan Studio, saya ingin bertanya mengenai layanan undangan digital.",
        )}
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Hubungi Admin melalui WhatsApp"
        className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] right-4 z-[45] inline-flex items-center gap-2 rounded-full bg-[#25D366] px-4 py-3 text-sm font-extrabold text-white shadow-[0_12px_35px_rgba(37,211,102,.38)] transition hover:-translate-y-0.5 hover:bg-[#20bd5a] hover:shadow-[0_16px_40px_rgba(37,211,102,.46)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-300 active:translate-y-0 sm:bottom-6 sm:right-6"
      >
      ...
      <footer className="mt-16 border-t border-white/10 bg-slate-950 px-4 py-9 text-center text-xs text-slate-400 sm:px-8">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-5 sm:flex-row">
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-left"><span className="block font-brand text-xl font-bold text-white leading-none">Undangan Studio</span><span className="block text-[10px] text-slate-500 mt-1">&copy; {new Date().getFullYear()} · Semua hak dilindungi</span></div>
          </div>
