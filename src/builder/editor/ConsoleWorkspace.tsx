...
} from "@/templates/navigation/protocol";
...
} from "./components/TabbedInspectorSidebar";
...
function canonicalizeSectionRecords(
  template: TemplateKit,
  records: StoredSectionRecord[],
) {
  ...
  return records.filter((record) => {
    ...
    const definition = template.sections.find(
      (section) => section.type === record.type,
    );
...
function getCanonicalSharedSectionOrder(
  template: TemplateKit,
  sectionsMap: Y.Map<unknown>,
  sectionOrder: string[],
) {
  const candidates = [
    ...sectionOrder,
    ...Array.from(sectionsMap.keys()).filter(
  ...
  for (const id of candidates) {
    ...
    const sectionType =
      typeof sectionMap.get("type") === "string"
        ? (sectionMap.get("type") as string)
        : "";
    const definition = template.sections.find(
      ...
      ? [
          {
            ...
            defaultData: { ...section.defaultData, ...record.data },
          },
        ]
      : [];
  });
...
function getSectionIcon(type: string) {
  ...
  if (t.includes("envelope") || t.includes("amplop"))
    return (
      <Mail
        size={13}
        className="shrink-0 text-slate-400 group-hover:text-emerald-600 transition-colors"
      />
    );
  if (
    t.includes("hero") ||
    ...
    return (
      <Heart
        size={13}
        className="shrink-0 text-slate-400 group-hover:text-rose-500 transition-colors"
      />
    );
  if (
    t.includes("event") ||
  ...
  if (t.includes("gift") || t.includes("hadiah") || t.includes("kado"))
    return (
      <Gift
        size={13}
        className="shrink-0 text-slate-400 group-hover:text-amber-500 transition-colors"
      />
    );
  if (t.includes("story") || t.includes("cerita") || t.includes("quote"))
    return (
      <Scroll
        size={13}
        className="shrink-0 text-slate-400 group-hover:text-purple-500 transition-colors"
      />
    );
  return (
    <Layers
...
function SidebarAccordion({
  ...
  return (
    <section
      ...
      className={`w-full min-w-0 max-w-full rounded-2xl border border-slate-200 bg-white shadow-[0_8px_24px_rgba(15,23,42,.05)] ${open ? "overflow-y-visible" : "overflow-hidden"} ${className}`}
    >
      <button
        type="button"
        onClick={onToggle}
        className="flex w-full min-w-0 items-center gap-2.5 sm:gap-3 px-3.5 py-3 text-left transition hover:bg-slate-50"
        aria-expanded={open}
      >
        <span
          className={`grid h-8 w-8 sm:h-9 sm:w-9 shrink-0 place-items-center rounded-xl transition ${open ? "bg-emerald-600 text-white shadow-sm" : "bg-emerald-50 text-emerald-600"}`}
        >
          {icon}
        </span>
        <span className="min-w-0 flex-1">
          <strong className="block truncate text-xs font-extrabold uppercase tracking-[.12em] text-slate-800">
            {title}
          </strong>
          <small className="mt-0.5 block truncate text-[10px] font-medium text-slate-500">
            {subtitle}
          </small>
        </span>
        <ChevronDown
      ...
      <div
        className={`grid w-full min-w-0 max-w-full transition-[grid-template-rows] duration-300 ease-out ${open ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
      >
        <div
          style={open ? { overflowX: "clip" } : undefined}
          className={`w-full min-w-0 max-w-full min-h-0 ${open ? "overflow-visible" : "overflow-hidden"}`}
        >
          <div className="w-full min-w-0 max-w-full border-t border-slate-100 p-2.5 sm:p-3.5">
            {children}
          </div>
        </div>
      </div>
...
export function ConsoleWorkspace({
  ...
  const [inspectorTab, setInspectorTab] =
    useState<InspectorSidebarTab>("section");
  ...
  const applySharedState = useCallback(
    (remoteState: SharedDraftState) => {
      ...
      if (
        ...
        Object.keys(remoteState.sections).length > 0
      ) {
        setSections(() => {
          const remoteSecMap = remoteState.sections;
          const orderedIds = [
            ...(remoteState.sectionOrder ?? []),
            ...Object.keys(remoteSecMap).filter(
              (id) => !(remoteState.sectionOrder ?? []).includes(id),
            ),
          ];
  ...
  useEffect(() => {
    ...
    collabDoc.updateLocalState((doc) => {
      ...
      const existingTypes = new Set(
        Array.from(sectionsMap.values()).flatMap((value) =>
          value instanceof Y.Map && typeof value.get("type") === "string"
      ...
      additions.forEach(({ id, definition }) => {
        ...
        Object.entries(definition.defaultData).forEach(([key, value]) =>
  ...
  useEffect(() => {
    ...
    collabDoc.updateLocalState((doc) => {
      ...
      const canonicalOrder = getCanonicalSharedSectionOrder(
        template,
        sectionsMap,
        currentOrder,
      );
      const retainedIds = new Set(canonicalOrder);
      const duplicateKnownIds = Array.from(sectionsMap.entries()).flatMap(
        ([id, sectionMap]) => {
          if (retainedIds.has(id) || !(sectionMap instanceof Y.Map)) return [];
          const type = sectionMap.get("type");
          return typeof type === "string" &&
            template.sections.some((section) => section.type === type)
            ? [id]
            : [];
        },
      );
      ...
      collabDoc.updateLocalState((doc) => {
        ...
        if (key === "themeId") {
          ...
          if (customColorsMap instanceof Y.Map) {
            Array.from(customColorsMap.keys()).forEach((k) =>
              customColorsMap.delete(k),
            );
          }
        } else if (key === "customColors") {
  ...
  function handleCustomColorChange(
    colorKey: "primary" | "accent" | "background",
    value: string,
  ) {
    ...
    (sectionId: string) => {
      ...
      setSections((items) => {
        ...
        if (target) {
          collabDoc.updateLocalState((doc) => {
            const sectionsMap = doc.getMap("sections");
            let secMap = sectionsMap.get(sectionId) as
  ...
  const theme =
    template.themes.find((item) => item.id === themeId) ?? template.themes[0];
  ...
  useEffect(() => {
    ...
    const savedStructureCollapsed = window.localStorage.getItem(
      `undangan-console:structure-collapsed:${template.code}`,
    );
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      ...
      const row = Array.from(
        panel?.querySelectorAll<HTMLElement>("[data-section-id]") ?? [],
      ).find((item) => item.dataset.sectionId === selectedId);
      if (panel && row) {
        const panelRect = panel.getBoundingClientRect();
        const rowRect = row.getBoundingClientRect();
        if (rowRect.top < panelRect.top + 12)
          panel.scrollTop -= panelRect.top + 12 - rowRect.top;
        else if (rowRect.bottom > panelRect.bottom - 12)
          panel.scrollTop += rowRect.bottom - (panelRect.bottom - 12);
      }
  ...
  useEffect(() => {
    ...
    const readLocalSnapshot = () => {
      ...
      try {
        ...
        return typeof parsed?.themeId === "string" && Array.isArray(parsed.sections)
          ? parsed
          : null;
      } catch {
        return null;
    ...
    const applyState = (
      nextThemeId: string,
      nextMusicUrl: string,
      records: LocalDraftSnapshot["sections"],
      nextCustomColors?: {
        primary?: string;
        accent?: string;
        background?: string;
      ...
      setMusicVolume(
        typeof nextMusicVolume === "number" ? nextMusicVolume : 0.6,
      );
    ...
    async function loadServerDraft(id: string) {
      ...
      if (nextStatus === "custom") {
        setPublishNotice({
          tone: "custom",
          message: customRequestIdentifier
            ? `Request ${customRequestIdentifier} sedang menunggu proses admin.`
            : "Request custom sedang menunggu proses admin.",
        });
      } else {
    ...
    async function initializeDraft() {
      ...
      if (!currentUser) {
        ...
        if (requestedDraftId) {
          requestLogin(
            "Masuk dengan Google untuk membuka dan mengedit draft kolaborasi ini.",
          );
        }
        if (localSnapshot)
          applyState(
            localSnapshot.themeId,
            typeof localSnapshot.musicUrl === "string"
              ? localSnapshot.musicUrl
              : defaultMusic,
            localSnapshot.sections,
      ...
      const legacyDraftId = window.localStorage.getItem(
        `undangan-console:draft:${template.code}`,
      );
      if (legacyDraftId) {
        const claim = await fetch(`/api/drafts/${legacyDraftId}/claim`, {
          method: "POST",
        });
      ...
      if (
        !localSnapshot &&
        storedDraftId &&
        (await loadServerDraft(storedDraftId))
      )
        return;
      ...
      if (localSnapshot) {
        const created = await fetch("/api/drafts", {
          ...
          body: JSON.stringify({
            ...
            title: `${template.name} - Draft lokal`,
          }),
        });
        ...
        await fetch(`/api/drafts/${createdPayload.draftId}`, {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            themeId: localSnapshot.themeId,
            settings: {
              musicUrl: localSnapshot.musicUrl,
  ...
  const presence = usePresence({
    ...
    enabled: Boolean(draftId && currentUser),
    role:
      (liveCollaborationRole as "owner" | "editor" | "viewer") ||
  ...
  const { status: autoSaveStatus, flush: flushAutoSave } = useAutoSave({
    ...
    onError: () => {
      if (presence.connectionStatus !== "connected") {
        setUploadError(
          "Gagal menyimpan perubahan ke server. Periksa koneksi Anda.",
        );
      }
    },
  });
    ...
    const receivePreviewMessage = (event: MessageEvent) => {
      ...
      if (event.data.type === "section-selected") {
        const section = sections.find(
          (item) => item.type === event.data.sectionType,
        );
        if (section) {
          ...
          window.requestAnimationFrame(() => {
            ...
            if (previewPanelRef.current)
              previewPanelRef.current.scrollTop = previewPanelScrollTop;
  ...
  useEffect(() => {
    ...
    fetch(`/api/wishes?invitationId=${encodeURIComponent(draftId)}`, {
      cache: "no-store",
      ...
      .finally(() => {
        if (!controller.signal.aborted) setWishesLoading(false);
      });
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    setSections((items) => {
      ...
      if (oldIndex < 0 || newIndex < 0 || !items[oldIndex].reorderable)
        return items;
  ...
  const updateSectionFields = useCallback(
    (sectionId: string, values: Record<string, unknown>) => {
      if (isViewer) return;
      setSections((items) =>
        items.map((sec) =>
          sec.id === sectionId
            ? { ...sec, defaultData: { ...sec.defaultData, ...values } }
            : sec,
        ),
      );
    ...
    (
      ...
      setSections((items) =>
        items.map((section) => {
          ...
          const current =
            section.defaultData.textStyles &&
            typeof section.defaultData.textStyles === "object"
              ? (section.defaultData.textStyles as Record<
                  string,
                  EditableTextStyle
                >)
              : {};
      ...
      collabDoc.updateLocalState((doc) => {
        ...
        let secMap = sectionsMap.get(sectionId) as Y.Map<unknown> | undefined;
        if (!secMap) {
          const currentSec = sections.find(
        ...
        if (!replace && previousStyle instanceof Y.Map) {
          previousStyle.forEach((value, styleKey) =>
            fieldStyleMap.set(styleKey, value),
          );
        } else if (
          !replace &&
          previousStyle &&
          ...
          Object.entries(previousStyle as Record<string, unknown>).forEach(
            ([styleKey, value]) => fieldStyleMap.set(styleKey, value),
          );
        }
  ...
  const updateSelectedTextStyle = useCallback(
    (key: string, style: Partial<EditableTextStyle>, replace = false) => {
      if (!selected) return;
  ...
  const activeFieldCollaborator = useCallback(
  ...
  async function uploadAsset(
    ...
    const response = await fetch(`/api/drafts/${currentDraftId}/assets`, {
      method: "POST",
      body: form,
    });
    const payload = await response
      .json()
      .catch(() => ({ error: "Respons upload tidak valid." }));
    ...
    const contentImageLimit =
      targetSectionType === "gallery"
        ...
        : Math.min(
            ...
            Math.max(
              1,
              typeof selected.defaultData.imageMax === "number"
                ? selected.defaultData.imageMax
                : 1,
            ),
          );
    const usesImageCollection =
      targetSectionType === "gallery" || contentImageLimit > 1;
    const selectedFiles = files.slice(
      0,
      usesImageCollection && target === "content" ? contentImageLimit : 1,
    );
    setUploadError("");
    setIsUploading(true);
    try {
      const uploaded = await Promise.all(
        selectedFiles.map((file) =>
          uploadAsset(file, targetSectionId, currentDraftId),
        ),
      );
      ...
      } else if (usesImageCollection) {
        const current = Array.isArray(selected.defaultData.imageUrls)
          ? (selected.defaultData.imageUrls as string[])
          : [];
        const imageUrls = [...current, ...uploaded.map((a) => a.url)].slice(
          -contentImageLimit,
        );
        updateSelectedFields({
          imageUrls,
          imageLabel: `${imageUrls.length} foto dipilih`,
        });
      } else {
        updateSelectedFields({
  ...
  function changeView(nextView: View) {
    if (nextView === "wishes" && !currentUser) {
      requestLogin(
        "Masuk dengan Google untuk melihat ucapan milik undangan yang Anda kelola.",
      );
      return;
    }
    setView(nextView);
  ...
  function canUpload(reason: string) {
    ...
    if (!draftId || !draftReady) {
      setUploadError(
        "Draft akun sedang disiapkan. Tunggu sebentar lalu coba lagi.",
      );
      return false;
    }
    return true;
  ...
  async function handleMusicSelection(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!currentUser) {
      requestLogin(
        "Masuk dengan Google terlebih dahulu untuk mengunggah musik undangan.",
      );
      return;
    }
    ...
    try {
      ...
      setUploadError(
        error instanceof Error ? error.message : "Upload musik gagal.",
      );
    } finally {
      setIsUploading(false);
    }
  ...
  function selectLibraryAsset(asset: UserAsset) {
    ...
    if (target.target === "background") {
      ...
      const configuredImageLimit =
        typeof selected?.defaultData.imageMax === "number"
          ? selected.defaultData.imageMax
          : 1;
      if (selected?.type === "gallery" || configuredImageLimit > 1) {
        const current = Array.isArray(selected.defaultData.imageUrls)
          ? (selected.defaultData.imageUrls as string[])
          : [];
        const imageLimit =
          selected.type === "gallery"
            ? 4
            : Math.min(4, Math.max(1, configuredImageLimit));
        const imageUrls = [
  ...
  function handlePublishResult(result: PublishResult) {
    ...
    if (result.status === "published") {
      ...
      setPublishNotice({
        tone: "success",
        message:
          "Selamat, undangan Anda telah berhasil dipublish. Silakan ke halaman Generator untuk memberi nama dan membagikan undangan kepada tamu pertama Anda.",
      });
      return;
    }
    ...
    setPublishNotice({
      ...
      message:
        result.mode === "subdomain"
          ? "Request subdomain berhasil dibuat. Status undangan kini Custom dan akan dikonfirmasi oleh admin."
          : "Request custom domain berhasil dibuat. Lanjutkan koordinasi konfigurasi domain bersama admin.",
    });
  }
      ...
      >
        <header className="sticky top-0 z-40 flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur md:px-6">
          <div className="flex min-w-0 items-center gap-3">
            <Link
              href="/"
              className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-xl transition hover:scale-105"
          ...
          <nav className="hidden xl:flex items-center rounded-xl border border-slate-200/80 bg-slate-100/90 p-1 shadow-xs shrink-0">
            {(
              [
                ["editor", "Editor", LayoutPanelTop],
                ["generator", "Generator", Send],
                ["wishes", "Ucapan", MessageCircleHeart],
              ] as const
            ).map(([id, label, Icon]) => (
              <button
                key={id}
              ...
              >
                <Icon
                  size={12}
                  className={
                    view === id ? "text-emerald-600" : "text-slate-400"
                  }
                />
                <span>{label}</span>
              </button>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {currentUser && draftId && (
              <div className="flex items-center gap-1.5 mr-1">
                <CollaborationStatus
                  status={presence.connectionStatus}
            ...
            {isOwner && presence.onlineCount === 0 && (
              <button
                ...
                onClick={() => {
                  if (!currentUser) {
                    requestLogin(
                      "Masuk dengan Google untuk mengundang kolaborator.",
                    );
                  } else {
            ...
            <UserAuthDropdown
              ...
              onLoginClick={() =>
                requestLogin(
                  "Masuk dengan Google untuk menyimpan dan mengelola undangan Anda.",
                )
              }
              onLogout={() => {
                setCurrentUser(null);
            ...
            {isOwner ? (
              <button
                ...
                onClick={() => {
                  if (!authResolved || !currentUser)
                    requestLogin(
                      "Masuk dengan Google untuk menyimpan dan mempublish undangan Anda.",
                    );
              ...
              >
                ...
                <span>
                  {draftStatus === "published" ? "Published" : "Dibagikan"}
                </span>
              </span>
            )}
          </div>
        ...
        {(!isOnline || presence.connectionStatus === "connecting") &&
          authResolved &&
          currentUser &&
          draftId && (
            <div className="fixed top-18 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 rounded-full border border-amber-300 bg-amber-50/95 px-4 py-1.5 text-xs font-bold text-amber-900 shadow-lg backdrop-blur-md animate-in slide-in-from-top-2 duration-200">
              ...
              <span>
                {!isOnline
                  ? "Anda sedang offline. Perubahan disimpan secara lokal."
        ...
        {view === "editor" && (
          ...
          >
            ...
            >
              <RemoteCursorLayer
                cursors={presence.remoteCursors}
                surface="left-sidebar"
              />
              ...
              <div className="mb-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-[10px] text-slate-500">
                    Geser section untuk mengatur urutan.
                  </p>
                </div>
            ...
            >
              ...
              >
                ...
                <div
                  role="group"
                  aria-label="Kontrol zoom kanvas"
                  className="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-xs"
                >
                  <button
                    type="button"
                  ...
                  <span className="pl-1 text-[10px] font-bold uppercase tracking-[.08em] text-slate-400">
                    Zoom
                  </span>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setZoomScale((z) =>
                        Math.max(0.65, Number((z - 0.1).toFixed(2))),
                      )
                    }
                  ...
                  <button
                    ...
                    onClick={() =>
                      setZoomScale((z) =>
                        Math.min(1.4, Number((z + 0.1).toFixed(2))),
                      )
                    }
              ...
              >
                ...
                {frameMode === "desktop" && (
                  <div
                    className="flex h-8 items-center border-b border-slate-200 bg-slate-50 px-3 text-slate-400"
                    aria-hidden="true"
                  >
                    ...
                    <span className="mx-auto pr-10 text-[10px] font-semibold uppercase tracking-[.12em]">
                      Desktop viewport
                    </span>
                  </div>
                )}
                <div className="relative">
                  ...
                  <iframe
                    ...
                    style={{
                      height:
                        frameMode === "desktop"
                          ? "clamp(640px, calc(100dvh - 210px), 900px)"
                          : "720px",
                    }}
            ...
            >
              {!isInspectorCollapsed && (
                ...
                >
                  <span className="absolute bottom-0 left-1/2 top-0 w-px -translate-x-1/2 bg-slate-200 transition group-hover:w-0.5 group-hover:bg-emerald-500 group-focus:w-0.5 group-focus:bg-emerald-600" />
                </div>
              )}
                    ...
                    <div className="relative flex items-center justify-between rounded-2xl border border-slate-200/90 bg-white/95 p-1.5 shadow-[0_8px_24px_rgba(15,23,42,.08)] backdrop-blur-md">
                      ...
                      <div className="relative flex items-center gap-1">
                        ...
                        >
                          {isSavingActive ? (
                            ...
                            <Check
                              size={15}
                              className="text-emerald-600 animate-in zoom-in-50 duration-150"
                            />
                          ) : (
                            <Save size={15} />
                          )}
                      ...
                      {isShortcutHelpOpen && (
                        <div className="absolute right-1.5 top-[calc(100%+8px)] z-50 w-52 rounded-xl border border-slate-200 bg-white p-3 shadow-xl">
                          <p className="mb-2 text-[10px] font-extrabold uppercase tracking-[.12em] text-slate-500">
                            Pintasan
                          </p>
                          <div className="space-y-1.5 text-[11px] font-medium text-slate-600">
                            <p className="flex items-center justify-between gap-3">
                              <span>Simpan</span>
                              <kbd className="rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[10px] font-bold text-slate-700">
                                Ctrl S
                              </kbd>
                            </p>
                            <p className="flex items-center justify-between gap-3">
                    ...
                    <div className="mt-3 min-w-0">
                      <CollaborativeSectionInspector
                        ...
                        onOpenContentLibrary={() =>
                          openAssetLibrary("image", "content")
                        }
                        onOpenBackgroundLibrary={() =>
                          openAssetLibrary("image", "background")
                        }
                      />
                    </div>
                  }
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 pb-6">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Pilih gaya bahasa undangan, lihat pratinjau pesan personal,
                  ...
                  {(
                    [
                      [
                        "formal",
                        "Formal & Santun",
                        "Cocok untuk keluarga & rekan kerja",
                      ],
                      [
                        "islami",
                        "Nuansa Islami",
                    ...
                    >
                      <b
                        className={`block text-xs font-extrabold ${waPreset === presetKey ? "text-emerald-950" : "text-slate-800"}`}
                      >
                        {label}
                      </b>
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="text-xs font-bold text-indigo-950">
                          Permintaan Custom Domain / Subdomain Diproses
                        </p>
                      ...
                      <p className="mt-0.5 text-[11px] text-indigo-800/80">
                        Fitur share WhatsApp akan aktif setelah domain/subdomain
                        resmi Anda (
                        <code className="font-mono font-bold text-indigo-950">
                          {publishIdentifier}
                        </code>
                        ) selesai dikonfirmasi dan dipublish oleh admin.
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <a
                      href={makeAdminWhatsAppUrl(
                        `Halo Admin, saya ingin menindaklanjuti status custom domain ${publishIdentifier} untuk draft ${draftId ?? "undangan saya"}.`,
                      )}
                      target="_blank"
                      rel="noreferrer"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-amber-950">
                        Undangan Belum Dipublish
                      </p>
                      <p className="text-[11px] text-amber-800/80">
                        Publish undangan terlebih dahulu agar tautan kirim
                        WhatsApp menggunakan domain/path resmi Anda.
                      </p>
                    </div>
                  </div>
                  <button
                    ...
                    onClick={() => {
                      if (!currentUser)
                        requestLogin(
                          "Masuk dengan Google dan publish undangan Anda.",
                        );
                      else setIsPublishOpen(true);
                    }}
                ...
                <div className="mt-6 flex items-center justify-between rounded-2xl border border-emerald-200 bg-emerald-50/80 px-4 py-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <p className="text-xs font-bold text-emerald-950">
                      Undangan Aktif & Siap Dibagikan ·{" "}
                      <span className="font-mono text-[11px] text-emerald-800 font-semibold">
                        {publishUrl || buildInvitationUrl(publishIdentifier)}
                      </span>
                    </p>
                  </div>
                  <a
              ...
              <div className="mt-6 border-t border-slate-100 pt-6">
                <div className="mb-1 flex items-center justify-between">
                  <p className="text-xs font-bold text-slate-800">
                    3. Daftar Tamu Undangan Terdaftar
                  </p>
                  <span className="text-[10px] font-semibold text-slate-400">
                    Tamu wajib terdaftar untuk menghindari manipulasi URL
                  </span>
                </div>
                ...
                <BulkGuestManager
                  ...
                  onRequirePublish={() => {
                    if (!currentUser)
                      requestLogin(
                        "Masuk dengan Google dan publish undangan Anda terlebih dahulu.",
                      );
                    else setIsPublishOpen(true);
                  }}
                  getMessageForGuest={(name) =>
                    getWhatsAppMessage(waPreset, name)
                  }
                />
              </div>
            </div>
        ...
        {view === "wishes" && (
          <section className="mx-auto max-w-3xl px-5 py-12">
            <div className="rounded-3xl border border-[#e5d7c8] bg-[#fffaf1] p-7 shadow-sm md:p-10">
              <div className="flex items-end justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-[#806f67]">
                    Ucapan tamu tersimpan di MySQL khusus untuk undangan ini.
                  </p>
                </div>
                <span className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-extrabold text-emerald-700">
                  {wishRecords.length} ucapan
                </span>
              </div>
              {wishesLoading ? (
                <div className="mt-7 grid place-items-center rounded-2xl border border-dashed border-[#d9c9b8] bg-white/70 p-10 text-sm text-[#9a887d]">
                  <LoaderCircle
                ...
                <div className="mt-7 rounded-2xl border border-dashed border-[#d9c9b8] bg-white/70 p-10 text-center text-sm text-[#9a887d]">
                  <MessageCircleHeart
                    className="mx-auto mb-3 text-emerald-600"
                    size={30}
                  />
                  Belum ada ucapan pada undangan ini.
                </div>
              ) : (
                <div className="mt-7 space-y-3">
                  {wishRecords.map((wish) => (
                    <article
                      key={wish.id}
                      className="rounded-2xl border border-[#eadfd5] bg-white p-4 shadow-sm"
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <strong className="text-sm text-[#4f3034]">
                          {wish.name}
                        </strong>
                      ...
                      <p className="mt-2 text-sm leading-6 text-[#75645f]">
                        {wish.message}
                      </p>
                      <time className="mt-3 block text-[10px] text-[#a08c82]">
        ...
        <PublishModal
          ...
          onResult={handlePublishResult}
        />

        {publishNotice && (
          <div
            className={`fixed left-1/2 top-20 z-[75] w-[min(92vw,620px)] -translate-x-1/2 rounded-2xl border p-4 shadow-[0_20px_60px_rgba(15,23,42,.24)] backdrop-blur ${publishNotice.tone === "success" ? "border-emerald-200 bg-emerald-50/95 text-emerald-950" : "border-amber-200 bg-amber-50/95 text-amber-950"}`}
            role="alert"
          >
            <div className="flex items-start gap-3">
              <span
                className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl ${publishNotice.tone === "success" ? "bg-emerald-600 text-white" : "bg-amber-500 text-white"}`}
              >
              ...
              <div className="min-w-0 flex-1">
                ...
                {publishNotice.tone === "success" ? (
                  <button
                    ...
                    className="mt-3 rounded-xl bg-emerald-700 px-3 py-2 text-[10px] font-extrabold text-white hover:bg-emerald-800"
                  >
                    Buka Generator
                  </button>
                ) : (
                  <a
                    href={makeAdminWhatsAppUrl(
                      `Halo Admin, saya ingin menindaklanjuti request custom ${publishIdentifier} untuk draft ${draftId ?? "saya"}.`,
                    )}
                    target="_blank"
                    rel="noreferrer"
                    className="mt-3 inline-flex items-center gap-1.5 rounded-xl bg-amber-600 px-3 py-2 text-[10px] font-extrabold text-white hover:bg-amber-700"
                  >
                    Hubungi admin, klik di sini! <ExternalLink size={12} />
                  </a>
                )}
              </div>
              <button
                ...
                className="rounded-full p-1 opacity-55 hover:bg-black/5 hover:opacity-100"
