...
export function CollaborativeSectionInspector({
  ...
  const [activeTextStyleField, setActiveTextStyleField] = useState<
    string | null
  >(null);
  ...
  const imageLimit =
    selected.type === "gallery"
      ...
      : Math.min(
          ...
          Math.max(
            1,
            typeof defaultData.imageMax === "number" ? defaultData.imageMax : 1,
          ),
        );
  const usesImageCollection = selected.type === "gallery" || imageLimit > 1;
  ...
  return (
    <div className="w-full min-w-0 max-w-full">
        ...
        {(selected.fields ?? [])
          .filter(
            (field) =>
              !(
                selected.type === "gift" &&
                ["bank2", "account2", "holder2"].includes(field.key)
              ),
          )
          .map((field) => {
            const value =
            ...
            const textStyles =
              ...
              typeof defaultData.textStyles === "object"
                ...
                : {};
            const legacyFonts =
              defaultData.fontStyles &&
              typeof defaultData.fontStyles === "object"
                ? (defaultData.fontStyles as Record<string, string>)
                : {};
            const style =
              textStyles[field.key] ??
              (legacyFonts[field.key]
                ? { fontFamily: legacyFonts[field.key] }
                : {});
            const collaborator =
              activeFieldCollaborator?.(selected.id, field.key) ?? null;
            ...
            return (
              <EditableField
                ...
                onTextStyleOpenChange={(open) =>
                  setActiveTextStyleField(open ? textStyleFieldId : null)
                }
      ...
      {selected.type === "gift" &&
        (
          [
            {
              key: "showBank",
              label: "Tampilkan rekening",
              description: "Sembunyikan data rekening tanpa menghapus isinya.",
              icon: "¤",
            },
            {
              key: "showQris",
              label: "Tampilkan QRIS",
              description: "Gambar tetap tersimpan saat disembunyikan.",
              icon: "▦",
            },
          ] as const
        ).map((control, index) => (
          <div
            key={control.key}
            className={`${index ? "mt-2" : "mt-5"} flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50/80 px-3.5 py-3`}
          >
            <div className="flex items-center justify-between gap-3">
              <span
                className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-violet-100 text-violet-700"
                aria-hidden="true"
              >
                {control.icon}
              </span>
              ...
              <button
                ...
                onClick={() =>
                  updateField(
                    selected.id,
                    control.key,
                    defaultData[control.key] === false,
                  )
                }
                className={`relative h-6 w-11 rounded-full border transition disabled:cursor-not-allowed disabled:opacity-50 ${
                  defaultData[control.key] !== false
                    ? "border-emerald-600 bg-emerald-600 shadow-sm shadow-emerald-200"
                    : "border-slate-300 bg-slate-200"
                }`}
              >
                <span
                  className={`absolute left-0.5 top-0.5 h-4.5 w-4.5 rounded-full bg-white shadow-sm transition-transform ${defaultData[control.key] !== false ? "translate-x-5" : "translate-x-0"}`}
                />
              </button>
              <span
                className={`text-[9px] font-extrabold uppercase tracking-[.1em] ${defaultData[control.key] !== false ? "text-emerald-700" : "text-slate-400"}`}
              >
                {defaultData[control.key] !== false ? "Aktif" : "Nonaktif"}
              </span>
            </div>
          </div>
        ))}
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            {defaultData.hasSecondAccount === true ? (
              <button
                ...
                disabled={isViewer}
                onClick={() =>
                  updateFields(selected.id, { hasSecondAccount: false })
                }
                className="rounded-lg px-2.5 py-1.5 text-[10px] font-extrabold text-rose-600 hover:bg-rose-50 disabled:opacity-50"
              >
                Hapus
              </button>
            ) : (
              <button
                ...
                disabled={isViewer}
                onClick={() =>
                  updateFields(selected.id, { hasSecondAccount: true })
                }
                className="rounded-lg bg-violet-600 px-3 py-1.5 text-[10px] font-extrabold text-white shadow-sm shadow-violet-200 hover:bg-violet-700 disabled:opacity-50"
              >
                + Tambah rekening
              </button>
            )}
              ...
              {[
                ...
                <label key={key} className="block">
                  ...
                  <input
                    ...
                    onBlur={() => broadcastFieldFocus?.(selected.id, null)}
                    onChange={(event) =>
                      updateField(selected.id, key, event.target.value)
                    }
      ...
      {(defaultData.imageLabel !== undefined ||
        selected.type === "gallery" ||
        Boolean(selected.capabilities?.gallery) ||
        Boolean(selected.capabilities?.image)) && (
        <div className="mt-5 border-t border-slate-100 pt-4">
          <AssetUploadField
            title={
              usesImageCollection
                ? `Foto komponen (Maks ${imageLimit})`
                : "Foto komponen"
            }
            urls={
              usesImageCollection
                ? Array.isArray(defaultData.imageUrls)
                  ? (defaultData.imageUrls.filter(
                      (url): url is string => typeof url === "string",
                    ) as string[])
                  : []
                : typeof defaultData.imageUrl === "string" &&
                    defaultData.imageUrl
                  ? [defaultData.imageUrl]
                  : []
            }
            hint={String(
              defaultData.imageLabel ||
                (usesImageCollection
            ...
            onRemove={(index) => {
              if (usesImageCollection) {
                const current = Array.isArray(defaultData.imageUrls)
                  ...
                  : [];
                const imageUrls = current.filter(
                  (_, itemIndex) => itemIndex !== index,
                );
                updateFields(selected.id, {
                  imageUrls,
                  imageLabel: imageUrls.length
      ...
      <div className="mt-5 border-t border-slate-100 pt-4">
        ...
        <div className="mb-3">
          <CollaborativeColorInput
            ...
            value={
              typeof defaultData.backgroundColor === "string"
                ? defaultData.backgroundColor
                : ""
            }
        ...
        <AssetUploadField
          ...
          urls={
            typeof defaultData.backgroundImageUrl === "string" &&
        ...
        {uploadError && (
          <p
            role="alert"
            className="mt-2 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-[10px] font-semibold leading-4 text-rose-700"
          >
            {uploadError}
          </p>
        )}
