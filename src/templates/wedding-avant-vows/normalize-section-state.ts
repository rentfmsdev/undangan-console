...
type SectionRecord = { id: string; type: string; enabled: boolean; data: Record<string, unknown> };

export function normalizeAvantVowsSectionState(template: TemplateKit, records: SectionRecord[], createId: (type: string) => string) {
  const known = records
